...
function GetPageData(pageNum, pageSize) {
    ...
    $.getJSON("Customer/Home/GetPage", { pageNumber: pageNum, pageSize: pageSize }, function (response) {
        ...
        for (var item of response.data) {
            ...
            let now = new Date();
            let create = new Date(item.createAt);
            const diff = new Date(now).getTime() - new Date(create).getTime();
            let discount = Math.floor(diff / (1000 * 60 * 60 * 24)) * 10;
            if (discount >= 50)
                discount = 50;
            let discountStr = '';
            if (discount > 0) {
                discountStr = `<span class="text-danger"> (-` + discount + `%)</span>`
            }
            rowData = rowData + `<div class="col-xl-2 col-lg-3 col-md-4 col-6">
                ...
                new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.price) + `/` + item.unit.name + discountStr + `</div> <!-- price-wrap.// -->
                        </figcaption>
                     </div>
                    </div> <!-- col.// -->`
        };
